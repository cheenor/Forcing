      program ISCCP
! try to open and read ISCCP D2 data
!C* SGI users:
...
C* ARRAYS TO HOLD ONE DECODED VARIABLE AS EQUAL-AREA MAP, AND SQUARE MAP
     ...
     $*******************************************************'/   
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!---------strings for filepath and name
      character*100 flpath,dir,flnm,fld2
	character year*4, mnsr*2,sr*1,fld1*7
	character sr2*18,sr3*1,typ*4, stime(9)*4    
!----------------------store raw data---------------------------------
      real raw(12,9,39,MAXLON,MAXLAT)	!!!! year month time vars lon lat 
	real Could(39,MAXBOX),CloudSQ(39,MAXLON,MAXLAT)         
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      dir='Z:\DATA\ISCCP\Freegz\'
	sr='\';sr3='.'
	typ='.GPC'
	stime(1)='0000'
	stime(2)='0300'
	...
	fld1='D2_'//year
	sr2='ISCCP.D2.0.GLOBAL.'
	ichen=0
      imm=ims
1986	if(ichen==1)then
	sr2='ISCCP.D2.1.GLOBAL.'
	ims=imm
	endif
	if(ichen==2)then
	sr2='ISCCP.D2.2.GLOBAL.'
	ims=imm
	endif
	if(ichen==3)then
	sr2='ISCCP.D2.3.GLOBAL.'
	ims=imm
	endif
	if(ichen==4)then
	sr2='ISCCP.D2.0.GLOBAL.'
	...
	ims=imm
	  ...
	  do 525 im=ims,12
	     write(mnsr,'(I2.2)')im
	     fld2=sr2//year//sr3//mnsr//typ
            do 417 it=1,9
	print*,month
	       flnm=sr2//year//sr3//mnsr//'.99.'//stime(it)//typ
             flpath=trim(dir)//fld1//sr//trim(fld2)//sr//trim(flnm) 
...
C*--------------------------------------------------------------------*C
c      open(10,file='Z:\DATA\ISCCP\Freegz\D2_1983\
c     1ISCCP.D2.0.GLOBAL.1983.07.GPC\
c     2ISCCP.D2.0.GLOBAL.1983.07.99.0000.GPC')

      LUND2 = 10                                                        
      CALL D2OPEN(IRC,flpath)  !-------------  input filename                                                
C* CHECK FOR OPEN ERROR 
	if(IRC/=0.and.ichen<8)then
	ichen=ichen+1
	print*,ichen,'**************8'
	imm=im
!	close(LUNANC)
...
C* FOR THIS SAMPLE PROGRAM, SAVE CLOUD AMOUNT IN A GLOBAL EQUAL AREA MAP
        if(iy==1984.and.im==7.and.it==9)then  !!! for test 
         EQMAP(IBOX) = RVAR(8,LON)
        endif
!!----------------------------------------------------------------------
!!!--------------------added the output variables you wanted------------
!!!---------the following is added by Chen jinghua-----------------------
	   Could(1,IBOX)  =   RVAR(8,LON)   !! cloud amount(CA)
	   Could(2,IBOX)  =   RVAR(29,LON)  !! MEAN CLOUD WATER PATH (WP)
	   Could(3,IBOX)  =   RVAR(32,LON)  !! MEAN low-level clouds
	   Could(4,IBOX)  =   RVAR(35,LON)  !! MEAN middle-level clouds
	   Could(5,IBOX)  =   RVAR(38,LON)  !! MEAN high-level clouds
!-------------cloud types--------------------------------------------------------------------------------------------------------
!------------ low level-----------------------------------------------------------------------------------------------------
	   Could(6,IBOX)  =   RVAR(41,LON)  !! Mean CA for cloud type 1 = Cumulus, liquid
	   Could(7,IBOX)  =   RVAR(45,LON)  !! Mean WP for cloud type 1
	   Could(8,IBOX)  =   RVAR(46,LON)  !! Mean CA for cloud type 2 = Stratocumulus, liquid
	   Could(9,IBOX)  =   RVAR(50,LON)  !! Mean WP for cloud type 2
	   Could(10,IBOX)  =   RVAR(51,LON)  !! Mean CA for cloud type 3 = Stratus, liquid
	   Could(11,IBOX)  =   RVAR(55,LON)  !! Mean WP for cloud type 3
	   Could(12,IBOX)  =   RVAR(56,LON)  !! Mean CA for cloud type 4 = Cumulus, ice
	   Could(13,IBOX)  =   RVAR(60,LON)  !! Mean WP for cloud type 4
	   Could(14,IBOX)  =   RVAR(61,LON)  !! Mean CA for cloud type 5 = Stratocumulus, ice
	   Could(15,IBOX)  =   RVAR(65,LON)  !! Mean WP for cloud type 5
	   Could(16,IBOX)  =   RVAR(66,LON)  !! Mean CA for cloud type 6 = Stratus, ice
	   Could(17,IBOX)  =   RVAR(70,LON)  !! Mean WP for cloud type 6
!-------- middle-level-------------------------------------------------------------------------------------------------------
	   Could(18,IBOX)  =   RVAR(71,LON)  !! Mean CA for cloud type 7 = Altocumulus, liquid
	   Could(19,IBOX)  =   RVAR(75,LON)  !! Mean WP for cloud type 7
	   Could(20,IBOX)  =   RVAR(76,LON)  !! Mean CA for cloud type 8 = Altostratus, liquid
	   Could(21,IBOX)  =   RVAR(80,LON)  !! Mean WP for cloud type 8
	   Could(22,IBOX)  =   RVAR(81,LON)  !! Mean CA for cloud type 9 = Nimbostratus, liquid
	   Could(23,IBOX)  =   RVAR(85,LON)  !! Mean WP for cloud type 9
	   Could(24,IBOX)  =   RVAR(86,LON)  !! Mean CA for cloud type 10 = Altocumulus, ice
	   Could(25,IBOX)  =   RVAR(90,LON)  !! Mean WP for cloud type 10
	   Could(26,IBOX)  =   RVAR(91,LON)  !! Mean CA for cloud type 11 = Altostratus, ice
	   Could(27,IBOX)  =   RVAR(95,LON)  !! Mean WP for cloud type 11
	   Could(28,IBOX)  =   RVAR(96,LON)  !! Mean CA for cloud type 12 = Nimbostratus, ice
	   Could(29,IBOX)  =   RVAR(100,LON)  !! Mean WP for cloud type 12
!------------hight level----------------------------------------------------------------------------------------------------
	   Could(30,IBOX)  =   RVAR(101,LON)  !! Mean CA for cloud type 13 = Cirrus
	   Could(31,IBOX)  =   RVAR(105,LON)  !! Mean WP for cloud type 13
	   Could(32,IBOX)  =   RVAR(106,LON)  !! Mean CA for cloud type 14 = Cirrostratus
	   Could(33,IBOX)  =   RVAR(110,LON)  !! Mean WP for cloud type 14
	   Could(34,IBOX)  =   RVAR(111,LON)  !! Mean CA for cloud type 15 = Deep convective
	   Could(35,IBOX)  =   RVAR(115,LON)  !! Mean WP for cloud type 15
!-----------------------------------------------------------------------------------------------------------------------------
         Could(36,IBOX)  =   RVAR(118,LON)  !! Mean RS from clear sky composite
	   Could(37,IBOX)  =   RVAR(128,LON)  !! Mean Precipitable water for 1000-680 mb (PW)
	   Could(38,IBOX)  =   RVAR(129,LON)  !! Mean Precipitable water for 680-310 mb (PW)
	   Could(39,IBOX)  =   RVAR(116,LON)  !! Mean Precipitable water for 680-310 mb (PW)
...
C* WRITE OUT THE SQUARE MAP
      if(iy==1984.and.im==7.and.it==9)then  !!!test for 1984 July monthly mean
      CALL EQ2SQ(1,EQMAP,SQMAP)                                             
      LUNOUT = 90                                                       
      OPEN(LUNOUT,file='90.dat'
     1,ACCESS='DIRECT',RECL=576,FORM='UNFORMATTED')    
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      OPEN(99,file='99.dat',form='binary')      
      DO 850 J=1,MAXLAT                                                 
         WRITE(LUNOUT,REC=J) (SQMAP(I,J),I=1,MAXLON)  
	   WRITE(99) (SQMAP(I,J),I=1,MAXLON)                  
  850 CONTINUE 
      endif    !!  test
C* WRITE OUT THE SQUARE MAP
      do icjh=1,39 !!!! the number of output           
      EQMAP(:)=Could(icjh,:)
	CALL EQ2SQ(1,EQMAP,SQMAP)
	CloudSQ(icjh,:,:)=SQMAP(:,:)
	enddo                                         
...
C**********************************************************************C
C**********************************************************************C
      SUBROUTINE D2OPEN(IRC,flpath) 
	character*100 flpath                                           
      COMMON /D2HEAD/ LUND2,IREC,IFILE,IYEAR,MONTH,IDAY,IUTC            
     $     ,LATBEG,LATEND,LONBEG,LONEND,IBXBEG,IBXEND                   
      IBXBEG = 0                                                        
      IBXEND = 0                                                        
      IREC = 0
	print*,flpath
	open(LUND2,file=trim(flpath)
     $,ACCESS='DIRECT',RECL=13000/4,                            
     $ FORM='UNFORMATTED',IOSTAT=IRC)
C      OPEN(LUND2,ACCESS='DIRECT',RECL=13000,                            
C     $     FORM='UNFORMATTED',IOSTAT=IRC)                               
      RETURN                                                            
      END                                                               
C**********************************************************************C
C**********************************************************************C
...
C**********************************************************************C
      ...
      COMMON /SQUARE/ LONLIM(2,MAXBOX)                                  
      CHARACTER*80    HEADER  
!	open(9,file='Z:\DATA\ISCCP\Software\Ancillary_Files_Needed.txt')                                          
      OPEN(LUNANC,file='Z:\DATA\ISCCP\Software\
     +Ancillary_Files_Needed.txt',IOSTAT=IRC)
C     +,	ACCESS='DIRECT',RECL=80,FORM='FORMATTED',IOSTAT=IRC)  
      IF ( IRC .NE. 0 ) RETURN                                          
!      READ(LUNANC,REC=1,FMT='(A80)') HEADER
!	print*, HEADER                            
!      READ(LUNANC,REC=2,FMT='(A80)') HEADER 
	READ(LUNANC,FMT='(A80)') HEADER
!	print*, HEADER                            
      READ(LUNANC,FMT='(A80)') HEADER 
!	print*, HEADER  
C	stop                           
      DO 100 IREC=1,MAXBOX                                              
C         READ(LUNANC,REC=IREC+2,FMT=110) IBOX,J,I,LONBEG,LONEND,   
          READ(LUNANC,FMT=110) IBOX,J,I,LONBEG,LONEND,         
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real raw(12,9,39,144,72) !!!! 1-144 west to east(-180-180?) /1-72 south to north(-90-90)
      real cld_jja(7,9,39)
!!!!-------------------------set ----------------------------------------------------
	  ...
	  do 100 iv=1,39  !!! varaiables
	   ...
	   XXX=0.0
	    do 101 im=ims,8
	     do 101 ln=lns,lne
	      do 101 lt=lts,lte 
	        if(raw(im,it,iv,ln,lt)==-1000.0)then
	        temp=temp
	        XXX=XXX+1.   !!!! the number of default value
			else
               temp=temp+ raw(im,it,iv,ln,lt)
	        endif
101   continue
           XYT=(8-ims+1)*1.0*(lne-lns+1)*(lte-lts+1)-XXX !!!!! take out default value
	     if(XYT==0)then
           cld_jja(ir,it,iv)=-9999.
	     else
	     cld_jja(ir,it,iv)=temp/XYT
	     endif
100   continue
      do ir=1,7
	write(999,801)area(ir),(cld_jja(ir,9,i),i=1,39)
...
!----------- addtional ------------------------------
     1,'RS_C','PW_L','PW_H','TS_K'
	do it=1,8
	write(tme,'(I2.2)')(it-1)*3
      write(998,802)tme,':00',(cld_jja(ir,it,i),i=1,39)
      enddo
	close(998)
	enddo
	close(999)
800   format(1X,A5,39(1X,A4))	
