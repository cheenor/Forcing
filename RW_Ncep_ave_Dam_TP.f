...
!--- you can change the data start date and end date in line 76 as need
...
!-------------------------------last modified 26/3/2012  Jinghua Chen
!---------the rainfall is pwr------------------------------------------------------- 
	include 'netcdf.inc'
...
!      real dat2d(15,10,17,nrecr) !(lon lat level time ivar) u,v,t w,,RH,hgt
      common/DD/ data3d(10,10,lev,nnr,n3d), !1u 1v 3t  4oemga 5RH 6HGT
...
!-------WTP=Weast TP   77.5-90   27.5 37.5 
 	 area(1)='WTP'; X1(1)=32; X2(1)=37; Y1(1)=26; Y2(1)=22;IXX(1)=6
       IYY(1)=5; bslat(1)=27.5; XG1(1)=42; XG2(1)=49; YG1(1)=33 
    	 YG2(1)=27
       IYG(1)=7; bslatG(1)=27.6186;IXG(1)=8;bslon(1)=77.5
!    East TP  90-102.5
	area(2)='ETP'; X1(2)=37; X2(2)=42 ;Y1(2)=26; Y2(2)=22; IXX(2)=6
	 IYY(2)=5; bslat(2)=27.5; XG1(2)=49;XG2(2)=56;YG1(2)=33;YG2(2)=27
       IYG(2)=7; bslatG(2)=27.6186;IXG(2)=8;bslon(2)=90.
...
C	area(4)='NEC'; X1(4)=48; X2(4)=52; Y1(4)=21; Y2(4)=17; IXX(4)=5
C      IYY(4)=5; bslat(4)=40.;XG1(4)=64;XG2(4)=69;YG1(4)=26; YG2(4)=21
C       IYG(4)=6; bslatG(4)=40.9517;IXG(4)=6;bslon(4)=117.5
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      var3d(1)='uwnd.'
	var3d(2)='vwnd.'
	var3d(3)='air.'
...
!---------------- make the output fold
	...
	dirout=trim(dir)//'TP\'//'NcepR2_Pre'
      istatus1=CHDIR(trim(dirout))
	istatus2=system("Md "//trim(foldout))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      
      do 1200 lo=2,3 ! start the area loop
...
!	if(lo==4) ikk=1
      ikk=10
!      if(lo==3) ikk=0
	 call DIVVOR(PLV,PLW,omegas,DIVO,IXX(lo),IYY(lo),bslat(lo),IT,
     +	 ikk,lo) !DIVVOR(PM,WSFC,DIVO,NXDIR,NYDIR,YMIN,IT)
       call VERVEL(PLV,PLW,omegas,DIVO,OMGO,IXX(lo),IYY(lo),IT,ikk,lo)
	 call ADJDV(PLV,PLW,DIVO,OMGO,DIV,IXX(lo),IYY(lo),IT,ikk,lo)
	 call VERVEL(PLV,PLW,omegas,DIV,OMG,IXX(lo),IYY(lo),IT,ikk,lo)
	 call Q1Q2(PLV,PLW,OMG,IXX(lo),IYY(lo),IT,lo,ikk)
	 call dynamic(data3d,OMG,DIV,IXX(lo),IYY(lo),bslat(lo),IT,dyn,lo) !IXX(lo),IYY(lo),bslat(lo),IT)
!----------------------------------------------------------------------------------------
       Tmcl=24*3600
       do ik=1,lev
...
CC        South North: the latitude array(x,y+1) is smaller than array(x,y)
      SUBROUTINE DIVVOR(PM,PW,WSFC,DIVO,NXDIR,NYDIR,YMIN,IT,ikk,lo)
	PARAMETER (GRID=2.50)
      integer,parameter :: nrec=1460
	integer,parameter :: nrecr=1464
	integer,parameter :: nnr=nrecr
...
CC  VERVEL COMPUTES VERTICAL VELOCITY
      SUBROUTINE VERVEL(PM,PW,WSFC,DIVO,OMGO,NXDIR,NYDIR,IT,ikk,lo)
	PARAMETER (GRID=2.50)
      integer,parameter :: nrec=1460
	integer,parameter :: nrecr=1464
	integer,parameter :: nnr=nrecr
	...
	integer NXDIR,NYDIR,it
     ...
     + PM(lev),OMGO(10,10,lev,nnr),PW(lev)
      common/DD/ data3d(10,10,lev,nnr,n3d), !1u 1v 3oemga 4t 5RH 6HGT
     +      data2d(10,10,nnr,n2d),hgt(10,10) ! !1lh 2sh 3pwr 4us 5vs 6 ts 7omegas 8Rhs 9Ps 10P_top
	COMMON/TQ/ tvs(10,10,nnr),qvs(10,10,nnr),
     +	tv(10,10,lev,nnr),qv(10,10,lev,nnr)
...
c      OMGO(I,J,2,it)=OMGO(I,J,1,it)+DIVO(I,J,2,it)
	OMGO(I,J,i1,it)=WSFC(I,J,it)
      OMGO(I,J,i2,it)=OMGO(I,J,i1,it)+DIVO(I,J,i2,it)
     + *(pres-PW(i2))
       IF(pres.GT.PM(i2))
     *    OMGO(I,J,i2,it)=OMGO(I,J,i1,it)+DIVO(I,J,4,it)*(pres-PW(i2))
	       if(isnan(OMGO(I,J,i2,it)))then
	        print*,OMGO(I,J,i2,it),DIVO(I,J,K,it)
	        print*,pres,PW(i2),1
	        stop
	        endif
      DO 110 K=i3,ktop-1      !!!! when the k=1ev, it is the tropopause, but actually it's not!!!! so need changed!!!
       IF(K.EQ.i3 .AND. pres.GT.PW(i2)) THEN
      OMGO(I,J,K,it)=OMGO(I,J,i1,it)+DIVO(I,J,K,it)*(pres-PW(i3))
             if(isnan(OMGO(I,J,K,it)))then
	        print*,OMGO(I,J,i1,it),DIVO(I,J,K,it)
	        print*,dp,k
	        print*,pres,PW(i3),2
	        stop
	        endif
	   ...
	   dp=pm(K-1)-pm(k)
         OMGO(I,J,K,it)=OMGO(I,J,K-1,it)+DIVO(I,J,K,it)*dp   !!!50 d of pm
	       if(isnan(OMGO(I,J,K,it)))then
	        print*,OMGO(I,J,K-1,it),DIVO(I,J,K,it)
	        print*,dp,k,3,i,j,it
	        stop
	        endif
       ENDIF
...
!	         tv_t=tv(ix,iy, ik, it+1)- tv(ix,iy, ik, it)
     ...
     +						 	*(tv_p/dp)	
        ...
        enddo
	 IF(pres.GT.PW(i2)) THEN
      OMGO(I,J,i2,it)=(OMGO(I,J,i1,it)+OMGO(I,J,i3,it))*0.5
	if(isnan(OMGO(I,J,i2,it)))then
	print*,OMGO(I,J,i1,it),5
	stop
	endif
       ENDIF
  200 CONTINUE
...
CC  ADJDV ADJUSTS DIVERGENCE AND VERTICAL VELOCITY USING O'BRIEN (1970)
      SUBROUTINE ADJDV(PM,PW,DIVO,OMGO,DIV,NXDIR,NYDIR,IT,ikk,lo)
      PARAMETER (GRID=2.50)
      integer,parameter :: nrec=1460
	integer,parameter :: nrecr=1464
	integer,parameter :: nnr=nrecr
...
!==================  BOB ==============================
	  ...
	  ELSE
!!!!!!---------Time boundary
	  if(IT==nnr.or.IT==(nnr-1))then
	STOS(M,N,K)=(data3d(M,N,K,IT,3)-data3d(M,N,K,IT-2,3))/(2.*DT) ! SN= next time tv; SP=previous time
      STOQ(M,N,K)=(QV(M,N,K,IT)-QV(M,N,K,IT-2))/(2.*DT)
	  else
      STOS(M,N,K)=(data3d(M,N,K,IT+2,3)-data3d(M,N,K,IT-2,3))/(4.*DT) ! SN= next time tv; SP=previous time
      STOQ(M,N,K)=(QV(M,N,K,IT+2)-QV(M,N,K,IT-2))/(4.*DT)
	   endif
	  ENDIF
...
!         ENDIF
       IF(K.EQ.kss.AND. psf.GE.PM(kss-1)) GO TO 850
         IF(K.EQ.kss.AND. psf.LT.PW(kss-1)) THEN
          UKM1=.5*(tv(M,N,K,It)-tv(M,N,K-1,IT))*OMG(M,N,1,IT)
          VKM1=.5*(QV(M,N,K,It)-QV(M,N,K-1,IT))*OMG(M,N,1,IT)
          DPW=PW(K)-psf
         ENDIF
         IF(K.EQ.kss.AND. psf.GE.PW(kss-1)) THEN
          UKM1=.5*(tv(M,N,K,IT)-tv(M,N,K-1,IT))*OMG(M,N,K-1,It)
          VKM1=.5*(QV(M,N,K,IT)-QV(M,N,K-1,IT))*OMG(M,N,K-1,IT)
         ENDIF
         IF(K.EQ.kss.AND. psf.GE.PM(kss-1)) THEN
           if(kss==3)DPW=PW(K)-psf
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	HAD_Q(I,J,K,IT)=-HADQ(I,J,K)
	VAD_Q(I,J,K,IT)=-VADQ(I,J,K)
     	TCH_Q(I,J,K,IT)=-STOQ(I,J,K)
...
C   20 CONTINUE
      ...
      END SUBROUTINE
C
      subroutine dynamic(d3d,OMGP,DIV,IX,IY,Blat,IT,dyn,lo) !IXX(lo),IYY(lo),bslat(lo),IT)
	PARAMETER (GRID=2.50)
	integer,parameter :: nrec=1460
	integer,parameter :: nrecr=1464
	integer,parameter :: nnr=nrecr
	...
	integer,parameter :: n3d=6 !1u 1v 3t  4oemga 5RH 6HGT
	real d3d(10,10,lev,nnr,n3d),OMG(10,10,lev,nnr)
      real OMGP(10,10,lev,nnr),blat
	real vor(10,10,lev,nnr,3),temp(4)
...
!------------------------------------------------
	...
	vorY=duy/dz-dwy*dxx
      if(dz==0)then
      print*,d3d(i,j,k,it,6),d3d(i,j,k-1,it,6)
	print*,k
	stop
...
200   continue
	...
	if(isnan(temp(ip)))then
	print*,vor(i,j,k,it,ip),ip
	stop	
	endif
	  enddo
...
103   continue
	...
	if(isnan(dyn(k,it,ip)))then
	print*,XY,temp(ip),ip,ix,iy
	stop	
	endif
	  enddo
	enddo
...
103   format(1X,I4,36(1X,e12.4))
      open(101,file='Z:\DATA\LargeScale\TP\NcepR2_Pre\NCEP_CTL\'
     +//year//'grads_Q.ctl')
...
197   continue
	XY=(IX-2.0)*(IY-2.)
	    ...
	    if(isnan(XYN_OUT(it,ik,ii)))then
           print*,ii
	     print*,XY
	     stop
	     endif
...
c	     if(i2d==9)then
c		 print*,data2d(ix-X1(lo)+1,iy-Y2(lo)+1,it+itt,i2d)
c            print*,fs(ix, iy,it ),scale4(1),add4(1)
c		 endif 
!		    if(i3d==4)then
!	        print*,data3d(ix-X1(lo)+1,iy-Y2(lo)+1,ik,it+itt,i3d)
!               endif
