...
!----------------------store raw data---------------------------------
      real raw(12,9,130,MAXLON,MAXLAT)	!!!! year month time vars lon lat 
	real Could(130,MAXBOX),CloudSQ(130,MAXLON,MAXLAT)         
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
C* WRITE OUT THE SQUARE MAP
      do icjh=1,130 !!!! the number of output           
      EQMAP(:)=Could(icjh,:)
	CALL EQ2SQ(1,EQMAP,SQMAP)
	CloudSQ(icjh,:,:)=SQMAP(:,:)
	enddo                                         
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real raw(12,9,130,144,72) !!!! 1-144 west to east(-180-180?) /1-72 south to north(-90-90)
      real cld_jja(7,9,130)
!!!!-------------------------set ----------------------------------------------------
	  ...
	  do 100 iv=1,130  !!! varaiables
	   ...
	   XXX=0.0
	    do 101 im=ims,8
	     do 101 ln=lns,lne
	      do 101 lt=lts,lte 
	          txx=raw(im,it,iv,ln,lt)
	        if(txx==-1000.or.txx==-9999.0)then
	         raw(im,it,iv,ln,lt)=0.0
	        temp=temp
...
101   continue
	     ...
	     if(XYT==0)then
           cld_jja(ir,it,iv)=0.0 !!! default value set zero
	     else
...
!      do ir=1,7
	 do it=1,9
 	  do iv=1,130
          write(999,99)(cld_jja(ir,it,iv),ir=1,7)
	  enddo
	 enddo
!	enddo
99    format(1X,7(1X,E16.8))
800   format(1X,A5,38(1X,A4))	
801   format(1X,A4,38(1X,F12.4))
802   format(1X,A2,A3,38(1X,F12.4)) 
      return
	end subroutine	
